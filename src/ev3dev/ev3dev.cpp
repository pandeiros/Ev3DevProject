...
namespace ev3dev
{
    ...
    const sensor::sensor_type sensor::custom_ultrasonic{"lego-ev3-us"};
    const sensor::sensor_type sensor::custom_gyro{"lego-ev3-gyro"};
    const sensor::sensor_type sensor::custom_color{"lego-ev3-color"};
    ...
    sensor(port_,{ev3_color, custom_color}) { }
    ...
    sensor(port_,{ev3_ultrasonic, nxt_ultrasonic, custom_ultrasonic}) { }
    ...
    sensor(port_,{ev3_gyro, custom_gyro}) { }
    ...
    sensor(port_,{nxt_sound, nxt_analog})
    {
        ...
        {
            device port;
            port.connect(SYS_ROOT "/class/lego-port/", "port",{
                {"port_name",
                    {port_name()}}
            });
    ...
    dc_motor::dc_motor(port_type port)
    {
        ...
        connect(_strClassDir, _strPattern,{
            { "port_name",
                { port}}
        });
    }
