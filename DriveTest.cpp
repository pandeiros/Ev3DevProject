...
protected:
    medium_motor _motor_left;
    //  large_motor     _motor_right;
    ultrasonic_sensor _sensor_ir;
...
{
    printf("SENSOR");
    _sensor_ir = ultrasonic_sensor(INPUT_1);
}
...
{
    std::cout << _motor_left.connected() << "\n";
    std::cout << _sensor_ir.connected() << "\n";
    //std::cout << _sensor_ir.value(0);
...
void control::drive_autonomously()
{
    ...
    _sensor_ir.set_mode(ultrasonic_sensor::mode_us_dist_cm);
